...
def train_one_epoch(model, data, loss, epoch, optimizer, scaler, scheduler, args, tb_writer=None):
    ...
    model.train()
    if args.extra_da:
        loss = Clip_DALoss(local_loss=args.local_loss, gather_with_grad=args.gather_with_grad, cache_labels=True, rank=args.rank, world_size=args.world_size, use_horovod=args.horovod, mar_loss=args.mar_loss, neg_loss=args.neg_loss, neg_loss_weight=args.neg_loss_weight, mar_loss_weight=args.mar_loss_weight, threshold_type=args.threshold_type, hardnegative=args.hardnegative)
    else:
        loss = ClipLoss(local_loss=args.local_loss, gather_with_grad=args.gather_with_grad, cache_labels=True, rank=args.rank, world_size=args.world_size, use_horovod=args.horovod)
    ...
    if args.mar_loss and args.threshold_type == 'fixed':
    ...
    for (i, batch) in enumerate(dataloader):
        ...
        if args.accum_freq == 1:
            with autocast():
                ...
                if args.extra_da:
                    (total_loss, thresholds, mar_loss, neg_loss) = loss(image_features, text_features, valid_caption_mask, logit_scale, thresholds)
        ...
        if is_master(args) and (i_accum % args.log_every_n_steps == 0 or batch_count == num_batches_per_epoch):
            ...
            logging.info(f"Train Epoch: {epoch} [{num_samples:>{sample_digits}}/{samples_per_epoch} ({percent_complete:.0f}%)] Loss: {loss_m.val:#.5g} ({loss_m.avg:#.4g}) Data (t): {data_time_m.avg:.3f} LR: {optimizer.param_groups[0]['lr']:5f} Logit Scale: {logit_scale_scalar:.3f} MAR_Loss: {(mar_loss if mar_loss is not None else None)} NEG_Loss: {(neg_loss if neg_loss is not None else None)} ")
            if args.mar_loss and args.threshold_type == 'fixed':
            ...
            log_data = {'loss': loss_m.val, 'data_time': data_time_m.val, 'batch_time': batch_time_m.val, 'samples_per_second': args.accum_freq * args.batch_size * args.world_size / batch_time_m.val, 'scale': logit_scale_scalar, 'lr': optimizer.param_groups[0]['lr'], 'MAR_Loss': mar_loss if mar_loss is not None else None, 'NEG_Loss': neg_loss if neg_loss is not None else None}
            if args.mar_loss and args.threshold_type == 'fixed':
...
def evaluate_winoground(model, data, epoch, args, tb_writer=None):
    ...
    if 'val' in data and (args.val_frequency and (epoch % args.val_frequency == 0 or epoch == args.epochs)):
        ...
        with torch.no_grad():
            ...
            metrics.update({**val_metrics, **winoground_metric, 'epoch': epoch, 'num_samples': num_samples})
    if not metrics:
        return metrics
...
def evaluate(model, data, epoch, args, tb_writer=None):
    ...
    if 'val' in data and (args.val_frequency and (epoch % args.val_frequency == 0 or epoch == args.epochs)):
        ...
        with torch.no_grad():
            ...
            val_metrics = get_clip_metrics(image_features=torch.cat(all_image_features), text_features=torch.cat(all_text_features), logit_scale=logit_scale.cpu())
