...
def parse_args(args):
    ...
    parser.add_argument('--train-data', type=str, help='Path to file(s) with training data')
    ...
    parser.add_argument('--val-num-samples', type=int, default=None, help='Number of samples in dataset. Useful for webdataset if not available in info file.')
    parser.add_argument('--dataset-type', choices=['webdataset', 'csv', 'synthetic', 'auto', 'npy', 'huggingface', 'json'], help='Which type of dataset to process.')
    parser.add_argument('--hardnegative', default=False, action='store_true', help='Whether to use da examples to compute clip loss')
    parser.add_argument('--threshold-type', choices=['mean', 'max', 'fixed'], default='mean', help='how to compute threshold')
    parser.add_argument('--fixed-threshold-value', type=float, default=2.0, help='fixed threshold value')
    parser.add_argument('--upper-bound', type=int, default=10, help='clamp upper bound for threshold')
    parser.add_argument('--positive-margin-loss', default=False, action='store_true', help='Enable dynamic margin loss for positive samples.')
    parser.add_argument('--positive-margin-loss-weight', type=float, default=0.5, help='Weight for the positive margin loss.')
    parser.add_argument('-neg', '--neg-loss', default=False, action='store_true', help='whether to use neg loss')
    parser.add_argument('--neg-loss-weight', type=float, default=0.2)
    parser.add_argument('-mar', '--mar-loss', default=False, action='store_true', help='Whether to use mar loss')
    parser.add_argument('--mar-loss-weight', type=float, default=0.2)
    ...
    parser.add_argument('--log-local', action='store_true', default=False, help='log files on local master, otherwise global master only.')
    ...
    parser.add_argument('--epochs-cooldown', type=int, default=None, help='When scheduler w/ cooldown used, perform cooldown from total_epochs - cooldown_epochs onwards.')
    ...
    parser.add_argument('--lr-scheduler', type=str, default='cosine', help="LR scheduler. One of: 'cosine', 'const' (constant), 'const-cooldown' (constant w/ cooldown). Default: cosine")
    parser.add_argument('--lr-cooldown-end', type=float, default=0.0, help='End learning rate for cooldown schedule. Default: 0')
    parser.add_argument('--lr-cooldown-power', type=float, default=1.0, help='Power for polynomial cooldown schedule. Default: 1.0 (linear decay)')
    parser.add_argument('--save-frequency', type=int, default=1, help='How often to save checkpoints.')
    parser.add_argument('--save-most-recent', action='store_true', default=False, help='Always save the most recent model trained to epoch_latest.pt.')
    ...
    parser.add_argument('--precision', choices=['amp', 'amp_bf16', 'amp_bfloat16', 'bf16', 'fp16', 'fp32'], default='amp', help='Floating point precision.')
    parser.add_argument('--model', type=str, default='ViT-B-32', help='Name of the vision backbone to use.')
    parser.add_argument('--pretrained', default='openai', type=str, help='Use a pretrained CLIP model weights with the specified tag or file path.')
    ...
    parser.add_argument('--wandb-project-name', type=str, default='open-clip', help='Name of the project if logging with wandb.')
    parser.add_argument('--debug', default=False, action='store_true', help='If true, more information is logged.')
    parser.add_argument('--copy-codebase', default=False, action='store_true', help='If true, we copy the entire base on the log directory, and execute from there.')
    ...
    parser.add_argument('--lock-text', default=False, action='store_true', help='Lock full text tower by disabling gradients.')
    parser.add_argument('--lock-text-unlocked-layers', type=int, default=0, help='Leave last n image tower layer groups unlocked.')
    parser.add_argument('--lock-text-freeze-layer-norm', default=False, action='store_true', help='Freeze BatchNorm running stats in image tower for any locked layers.')
    parser.add_argument('--log-every-n-steps', type=int, default=50, help='Log every n steps to tensorboard/console/wandb.')
    parser.add_argument('--remote-sync', type=str, default=None, help='Optinoally sync with a remote path specified by this arg')
    parser.add_argument('--remote-sync-frequency', type=int, default=300, help='How frequently to sync to a remote directly if --remote-sync is not None.')
    parser.add_argument('--remote-sync-protocol', choices=['s3', 'fsspec'], default='s3', help='How to do the remote sync backup if --remote-sync is not None.')
    parser.add_argument('--delete-previous-checkpoint', default=False, action='store_true', help='If true, delete previous checkpoint after storing a new one.')
