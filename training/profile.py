...
def profile_fvcore(model, image_input_size=(3, 224, 224), text_input_size=(77,), batch_size=1, detailed=False, force_cpu=False):
    if force_cpu:
        model = model.to('cpu')
...
def profile_model(model_name):
    ...
    while retries:
        retries -= 1
        try:
            (macs, acts) = profile_fvcore(model, image_input_size=image_input_size, text_input_size=text_input_size, force_cpu=not retries)
            (image_macs, image_acts) = profile_fvcore_image(model.visual, image_input_size=image_input_size, force_cpu=not retries)
            (text_macs, text_acts) = profile_fvcore_text(model.text, text_input_size=text_input_size, force_cpu=not retries)
            results['gmacs'] = round(macs / 1000000000.0, 2)
            results['macts'] = round(acts / 1000000.0, 2)
            results['mparams'] = round(count_params(model) / 1000000.0, 2)
            results['image_gmacs'] = round(image_macs / 1000000000.0, 2)
            results['image_macts'] = round(image_acts / 1000000.0, 2)
            results['image_mparams'] = round(count_params(model.visual) / 1000000.0, 2)
            results['text_gmacs'] = round(text_macs / 1000000000.0, 2)
            results['text_macts'] = round(text_acts / 1000000.0, 2)
            results['text_mparams'] = round(count_params(model.text) / 1000000.0, 2)
